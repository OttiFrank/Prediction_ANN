...
rain_values.reset_index(level=0, inplace=True)
rain_values
df = pd.merge(rain_values, pred_list, left_index=True, right_index=True)
...
pyplot.plot(df['Rain_mm_sum'], color='forestgreen', label='Regnmängd')
pyplot.xlabel('Tid')
pyplot.ylabel('millimeter')
pyplot.title('Marknivå och regnmängd under en treårsperiod mätt i millimeter')
pyplot.legend(['Predikterade värden', 'Riktiga värden', 'Regn'])
pyplot.show()
